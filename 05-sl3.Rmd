...
```{r screener, message=FALSE, warning=FALSE}
screen_rf <- make_learner(Lrnr_screener_randomForest, nVar = 5, ntree = 50)
...
```{r screener-pipe, message=FALSE, warning=FALSE}
...
```{r CVsl-faster, message=FALSE, warning=FALSE}
stack_smaller <- make_learner(Stack, 
  lrnr_glm, lrnr_mean, lrnr_lasso, lrnr_ridge, lrnr_elasticnet
)

sl_faster <- make_learner(Lrnr_sl,
  learners = stack_smaller
)
sl_faster_fit <- sl_faster$train(ist_task)
```
...
ist_task_new <- make_sl3_Task(
  ...
  covariates = covars,
  outcome = outcome,
  folds = make_folds(ist_data, fold_fun = folds_vfold, V = 2),
  drop_missing_outcome = TRUE
)
CVsl <- CV_lrnr_sl(sl_faster_fit, ist_task, loss_loglik_binomial)
CVsl %>%
  kable(digits = 4) %>%
  kable_styling(fixed_thead = T, font_size = 10) %>%
  scroll_box(width = "100%", height = "250px")
