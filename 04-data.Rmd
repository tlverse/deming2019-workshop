...
# read in data
dat <- read_csv("https://raw.githubusercontent.com/tlverse/deming2019-workshop/data/ist_sample.csv")
dat
```
...
after randomization (`DRSISC` in `dat`); the variable $\Delta$ indicates whether 
or not our outcome $Y$ is missing (`Delta` in `dat`); the treatment of interest, 
$A$, is the randomized aspirin vs. no aspirin treatment allocation (`RXASP` in 
`dat`); and the adjustment set, $W$, consists simply of other variable measured 
at baseline. This results in our observed data structure being $n$ i.i.d. copies 
of $O_i = (W_i, A_i, \Delta_i, Y_i)$, for $i = 1, \ldots, n$.
