...
### 2. Obtain intial estimates with `SuperLearner` `R` package 
...
```{r moss-3-marginal, warning=FALSE, message=FALSE}
...
```{r moss-4-ate, eval=FALSE, echo=FALSE}
# estimate and obtain inference for survival curve for ate
survival_ate <- survival_curve$new(t = k_grid, survival = psi_testA)
survival_curve_ate <- as.vector(survival_ate$survival)
